...
-  ```IgnoreMany(IEnumerable<string> propertiesToIgnore)``` Sets the destination properties with the names passed to be ignored during the mapping process. The value of these properties will stay the same as it was on the destination when the map function was called.
